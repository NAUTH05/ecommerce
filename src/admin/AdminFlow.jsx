...
function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    Promise.all([getManagedProducts(), getManagedCategories(), getUsers(), getOrders('all', true)])
      .then(([products, categories, users, orders]) => { if (!cancelled) setData({ products, categories, users, orders }); })
  ...
  return <div>
    ...
    <div className="admin-columns">
      <section className="admin-panel"><div className="admin-panel-head"><h3>Recent orders</h3><Link to="/admin/orders">View all</Link></div>{data.orders.slice(0, 5).map((order) => <Link className="admin-row" to={`/admin/orders/${order.id}`} key={order.id}><span>#{safeText(order.id).slice(0, 8)}<small>{order.customerName || order.email}</small></span><span className="status">{order.status}</span><strong>{money(order.total)}</strong></Link>)}{!data.orders.length && <p>No orders yet.</p>}</section>
...
function AdminProducts() {
  ...
  useEffect(() => {
    let cancelled = false;
    Promise.all([getManagedProducts(), getManagedCategories()])
      .then(([nextProducts, nextCategories]) => { if (!cancelled) { setProducts(nextProducts); setCategories(nextCategories); } })
      .catch((loadError) => { if (!cancelled) setError(loadError.message); });
    return () => { cancelled = true; };
  }, []);
  useEffect(() => { if (location.state?.message) window.history.replaceState({}, '', location.pathname); }, [location]);
  const shown = products.filter((product) => safeText(product.name).toLowerCase().includes(search.toLowerCase()) && (category === 'all' || product.categoryId === category));
...
function ProductForm() {
  ...
  useEffect(() => {
    let cancelled = false;
    getManagedCategories()
      .then((nextCategories) => { if (!cancelled) setCategories(nextCategories); })
      .catch((loadError) => { if (!cancelled) setError(loadError.message); });
    if (id) {
      getManagedProduct(id)
        .then((product) => {
          if (cancelled) return;
          if (!product) throw new Error('Product was not found.');
          setForm({ name: product.name || '', description: product.description || '', price: product.price ?? '', stock: product.stock ?? '', categoryId: product.categoryId || '', imageUrl: product.imageUrl || '' });
          setLoading(false);
        })
...
function AdminOrders() {
  ...
  useEffect(() => { let cancelled = false; getOrders('all', true).then((nextOrders) => { if (!cancelled) setOrders(nextOrders); }).catch((loadError) => { if (!cancelled) setError(loadError.message); }); return () => { cancelled = true; }; }, []);
...
function AdminOrderDetail() {
  ...
  useEffect(() => { let cancelled = false; getOrder(id).then((nextOrder) => { if (!cancelled) setOrder(nextOrder); }).catch((loadError) => { if (!cancelled) setError(loadError.message); }); return () => { cancelled = true; }; }, [id]);
